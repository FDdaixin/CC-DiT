# 图像预处理和增强的类和函数，包括图像缩放、翻转、标签转换、颜色映射等操作
import numpy as np
import torch
from PIL import Image
import collections

# 该类实现了图像处理中常见的图像缩放功能，支持按指定尺寸缩放图像，并可以选择不同的插值方法。
class Scale(object):
...
# 输入的图像或标签转换为 PyTorch 张量，并转换为浮点数类型。
class ToLabel(object):
    def __call__(self, inputs):
        return torch.from_numpy(np.array(inputs)).float()

# 输入中的所有 olabel 替换为 nlabel。
...
# 返回一个形状为 (N, 3) 的 NumPy 数组，其中每一行代表一个标签值对应的 RGB 颜色。
def labelcolormap(N):
