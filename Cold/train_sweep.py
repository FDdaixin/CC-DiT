...
def setup_logger(log_file: str):
    ...
    Unique per run (use log_file path as logger name).
    """
    logger = logging.getLogger(log_file)
    logger.setLevel(logging.INFO)
    logger.propagate = False

    # IMPORTANT: clear handlers to avoid duplicated logs when sweeping
    if logger.handlers:
...
def run_one_experiment(
    *,
    ...
    Saves logs to exp_dir/train.log
    """
    ...
    cfg = dict(
        ...
        betas=f"{adam_beta_1},{adam_beta_2}",  # string to avoid tensorboard hparams issues
    ...
    if accelerator.is_main_process:
        # Do NOT pass config to avoid add_hparams error
        accelerator.init_trackers("ColdSegDiffusion")

    if accelerator.is_main_process:
        logger.info(f"[RUN] dir={exp_dir}")
        logger.info(f"[LOSS] alpha={loss_alpha}, beta={loss_beta}, gamma={loss_gamma}")
        logger.info(f"[CFG] epochs={epochs}, img_size={img_size}, time_steps={time_steps}, overlap={overlap}")
    ...
    try:
        for epoch in range(epochs):
            ...
            if accelerator.is_main_process:
                logger.info(
                    f"Epoch {epoch+1}/{epochs} | train_loss={epoch_train_loss:.6f} | lr={lr:.3e} "
            ...
            if (epoch % val_every == 0):
                ...
                for idx, (images, masks, _) in enumerate(tqdm(valid_loader, disable=not accelerator.is_main_process)):
                    ...
                    merged_pred = merge_tiles(
                        predicted_tiles, pad_shape, (original_h, original_w),
        ...
        if accelerator.is_main_process:
            logger.info(f"[DONE] total_minutes={total_time/60:.2f}")
        ...
        if accelerator.is_main_process:
            try:
                ...
                plt.close()
                logger.info("[PLOT] saved train_loss_curve.png")
            except Exception as e:
                logger.warning(f"[WARN] loss plot failed: {e}")

    except Exception as e:
        # log stack trace into train.log
        if accelerator.is_main_process:
            logger.exception(f"[ERROR] training crashed: {e}")
        raise
    finally:
