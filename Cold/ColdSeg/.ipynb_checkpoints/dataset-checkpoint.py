...
class DRIVE_DataSet(Dataset):
    ...
    mask_transform=None  , training=None, flip_p=0.0):
        self.root = root
        self.split = split
        ...
        imgsets_dir = osp.join(root, split, "images")
        masksets_dir = osp.join(root, split, "labels")
        for name in os.listdir(imgsets_dir):  # 遍历imgsets_dir目录下的所有文件 name：“21_training.tif”
            name = os.path.splitext(name)[0] # splitext函数来从文件名中分离出文件名和扩展名，然后只保留文件名部分。"example.txt"，那么os.path.splitext(name)的结果将是('example', '.txt')。
            img_file = osp.join(imgsets_dir, "%s.tif" % name)
            label_file = osp.join(masksets_dir, "%s.gif" % name) # ..//08_training.gif
    ...
    def __getitem__(self, index):
        ...
        image_path = datafiles["images"]
        mask_path = datafiles["labels"]

        img = Image.open(datafiles["images"]).convert('RGB')
        mask = Image.open(datafiles["labels"]).convert('L')
...
class ISICDataset(Dataset):
    def __init__(self, data_path, dataset, is_folder, transform=None, training=True, flip_p=0.0):
        ...
        # Read txt file
        fh_txt = open(data_path + "\\" + dataset, 'r') #  r"D:\MyDataSet\ISIC_Med\train.txt"
        images_labels = []
        for line in fh_txt:
            line = line.rstrip()  # Default deletion is blank characters ('\n', '\r', '\t', ' ')
            images_labels.append(line)  #  images_labels = ['ISIC_0000000', 'ISIC_0000001', 'ISIC_0000002', ...]
        ...
        self.data_path = data_path + "\\" + is_folder  #  r"D:\MyDataSet\ISIC_Med\ISBI2016_ISIC_Dataset"
        self.images_labels = images_labels   #  images_labels = ['ISIC_0000000', 'ISIC_0000001', 'ISIC_0000002', ...]
    ...
    def __getitem__(self, index):
        ...
        name = self.images_labels[index] + '.jpg' # ['ISIC_0000000', 'ISIC_0000001', 'ISIC_0000002', ...] + ".jpg" 绝对路径
        img_path = os.path.join(self.data_path, name)   # r"D:\MyDataSet\ISIC_Med\ISBI2016_ISIC_Dataset\ISIC_0000000.jpg"
