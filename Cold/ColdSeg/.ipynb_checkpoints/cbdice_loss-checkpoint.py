...
from ColdSeg.skeletonize import Skeletonize
from ColdSeg.soft_skeleton import SoftSkeletonize
...
class SoftcbDiceLoss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true, t_skeletonize_flage=True):
        ...
        y_pred_fore = y_pred[:, 0:1]  # 取唯一通道
        y_pred_binary = torch.cat([1 - y_pred_fore, y_pred_fore], dim=1)  # 构造背景+前景
        y_prob_binary = torch.softmax(y_pred_binary, 1)
        y_pred_prob = y_prob_binary[:, 1]  # 提取前景概率
